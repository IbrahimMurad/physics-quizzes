...
def exam_create(request):
    ...
    if exam_type == "multi_scope":
        ...
        if len(scope_ids) > MAX_SCOPES:
            messages.error(
                request, f"Too many scopes selected. Maximum allowed is {MAX_SCOPES}"
            )
            return reload(request)
    ...
    try:
        ...
        if len(scopes) != len(valid_scope_ids):
            ...
            messages.error(
                request,
                f"Some scopes were not found: {', '.join(map(str, missing_ids))}",
            )
            return reload(request)
    ...
    if len(unique_problems) < target_problems:
        if len(unique_problems) == 0:
            messages.error(
                request, "No problems are available for the selected scope(s)"
            )
        else:
            messages.warning(
                request,
    ...
    if not exam_title or not exam_title.strip():
        if exam_type == "single_scope":
            ...
            scope_names = ", ".join([
                f"{scope.type}: {scope.title}" for scope in scopes[:2]
            ])
            if len(scopes) > 2:
                scope_names += f" and {len(scopes) - 2} more"
            exam_title = (
                f"Multi-scope exam ({scope_names}) - created by {request.user.username}"
            )
    ...
    try:
        ...
        messages.error(
            request, "An error occurred while creating the exam. Please try again."
        )
        return reload(request)
